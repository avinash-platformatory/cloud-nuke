...
Scheduled and on-demand cleanup of billable cloud resources in AWS, OCI, Azure, GCP, DigitalOcean, and Civo accounts.
...
- Civo uses `civo_api_token` only. Cleanup covers the **entire account** (all regions visible to the token), or a single `--region`. Object stores and object-store credentials are deleted; SSH keys, teams/permissions, and account API keys are **not**.
...
**Civo (entire account, or single `--region`):** Kubernetes clusters; instances; volumes; instance snapshots; load balancers; firewalls; networks; databases; object stores; object-store credentials; custom disk images (best-effort). SSH keys, teams/permissions, and account API keys are **not** deleted.
